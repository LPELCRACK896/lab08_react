...
class Game extends React.Component {
  ...
  clickRegisterCardSelected($event) {
    ...
    if ($event.target.className != 'front' && $event.target.className != null) {
      //SI tiene un ID valido pues
      if (
        !this.state.solved.includes(
          cards_info.find((card) => card.id == $event.target.className)
        ))
        {
        if (this.state.first_card_flipped == null) {
          ...
          this.setState({first_card_flipped: cards_info.find((card) => card.id == $event.target.className)})
          this.setState({count_cards_flipped: this.state.count_cards_flipped+1})
        } else {
          //Se esta volteando la segunda carta
          this.state.flipped.set(
            cards_info.find((card) => card.id == $event.target.className).id,
            true
          );
          if (
            this.state.first_card_flipped.pair_id ==
            cards_info.find((card) => card.id == $event.target.className).id
          ) {
            this.setState({coincidio: true})
            this.setState({points: this.state.points+1})
            this.state.solved.push(
              cards_info.find((card) => card.id == $event.target.className).id
            );
            this.state.solved.push(this.state.first_card_flipped.id);
          } else {
            //No coincide la segunda carta
            this.setState({coincidio: false})
            //Esto tengo que hacer que lo haga después de un tiempo, para que se vea la segunda carta
            this.resetFlip(
              cards_info.find((card) => card.id == $event.target.className).id,
              this.state.first_card_flipped.id
            );
          }
          this.setState({first_card_flipped: null})
        }
      }
    } 
      ...
      <>
        <header>
          ...
          <div className="div-text" id="moves-div">
            <div>Movimientos: {this.state.moves}</div>
          </div>
          <div className="div-text" id="record-div">
            <div>Puntos: {this.state.points}/6</div>
          </div>
        </header>
          ...
          <div id="game-grid">
            {this.state.setup.map((id) => {
              return (
                ...
                >
                  <Card cardid={id} flip={this.state.flipped.get(id)} />
                </div>
              );
            })}
            <div
